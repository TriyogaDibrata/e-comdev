<div class="max-w-screen-xl mx-auto py-10 px-6 md:px-10">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 items-start justify-around">
        ...
        <div class="p-4">
            <h2 class="text-2xl font-bold my-4 mx-2">Order Detail</h2>
            @foreach ($detail->orderItems as $item)
                <div class="p-4 mb-4 card-shadow flex flex-row gap-4 items-center rounded-md hover:cursor-pointer">
                    <img class="rounded-md w-28 h-28 object-cover" src="{{ $item->product->media[0]->original_url }}" />

                    <div class="flex flex-col">
                        <h3 class="text-md font-semibold">{{ $item->qty . ' x ' . $item->product->name }}</h3>
                        <div class="text-xs font-thin">Price :
                            {{ 'Rp. ' . number_format($item->product->price_per_unit, 2, ',', '.') }}</div>
                        <div class="text-semibold mt-3">Total :
                            {{ 'Rp. ' . number_format($item->qty * $item->product->price_per_unit, 2, ',', '.') }}
                        </div>
                    </div>
                </div>
            @endforeach
            ...
            <div class="w-full p-4 card-shadow rounded-md bg-orange-50">
                ...
                <div class="p-4 border-b">
                    <div class="flex flex-row justify-between">
                        <div>Total Price</div>
                        <div>{{ 'Rp. ' . number_format($detail->total, 2, ',', '.') }}</div>
                    </div>
                    <div class="flex flex-row justify-between">
                        <div>Tax (11%) </div>
                        <div>{{ 'Rp. ' . number_format($detail->tax, 2, ',', '.') }}</div>
                    </div>
                    <div class="flex flex-row justify-between">
                        <div class="text-lg font-semibold">Grand Total</div>
                        <div class="text-lg font-semibold">
                            {{ 'Rp. ' . number_format($detail->grand_total, 2, ',', '.') }}
                        </div>
                    </div>
                </div>
                ...
                <div class="p-4">
                    <div class="my-2">
                        <label for="payment-method" class="block text-sm font-medium text-gray-900">Ticket</label>
                        <div class="text-md font-semibold">{{ '#' . $detail->ticket }}</div>
                    </div>

                    <div class="my-2">
                        <label for="payment-method" class="block text-sm font-medium text-gray-900">Payment
                            Method</label>
                        <div class="text-md font-semibold">{{ $detail->paymentMethod->name }}</div>
                    </div>
                    ...
                    <div class="my-2">
                        ...
                        <div class="text-md font-semibold">
                            @switch($detail->payment_status)
                                @case(0)
                                    <div class="text-yellow-400 text-md font-semibold">
                                        Pending Confirmation
                                    </div>
                                @break

                                @case(1)
                                    <div class="text-green-400 text-md font-semibold">
                                        Payment Confirmed
                                    </div>
                                @break
                    ...
                    <div class="my-2">
                        <label for="payment-method" class="block text-sm font-medium text-gray-900">Address</label>
                        <div class="text-md font-semibold">{{ $detail->shipping_address }}</div>
                    </div>

                    <div class="my-2">
                        <label for="payment-method" class="block text-sm font-medium text-gray-900">Status</label>
                        @switch($detail->status)
                            ...
                            @case(1)
                                <div class="text-blue-400 text-md font-semibold">
                                    Processing
                                </div>
                            @break

                            @case(2)
                                <div class="text-purple-400 text-md font-semibold">
                                    Delivered
                                </div>
                            @break
                            ...
                            @case(4)
                                <div class="text-red-400 text-md font-semibold">
                                    Canceled
                                </div>
                            @break
